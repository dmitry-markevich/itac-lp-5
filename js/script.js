    ...
    $(document).ready(function () {
        ...
        function lpGoToActive() {
            ...
            if (lpPath != '') {
                lpTrgt = $('#' + lpPath);
                if (lpTrgt.length > 0) {
                    $('body, html').scrollTop(lpTrgt.offset().top - 40);
                }
            }
            setTimeout(function () {
                lpReady = true;
...
function lpMapInit() {
    ...
    var lpApp = new ymaps.Map('lp-map', lpMapOptions);
    ...
    var lpPlacemark = new ymaps.Placemark(lpMapOptions.center, {
        ...
        balloonContentFooter: 'пер. 4-й Загородныйб 56А'
    });

    lpApp.geoObjects.add(lpPlacemark);
}
